...
function normalisePlan(plan, subunit) {
  ...
  return { id, name, description, price, amount: Math.round(price * subunit), credits };
}
...
export class PaystackPayments {
  ...
  get config() {
    const channels = this.setting('PAYSTACK_CHANNELS', 'mobile_money,card').split(',').map(value => value.trim()).filter(Boolean);
    return {
  ...
  async initialize({ account, email, planId }) {
    const receiptEmail = cleanEmail(email || account?.email);
    if (!account?.id || receiptEmail !== String(account.email || '').toLowerCase()) {
      throw Object.assign(new Error('Use the email address on your signed-in Dresscode account.'), { statusCode: 403 });
    }
    const wallet = await this.store.ensureOwnedWallet(account);
    ...
    try {
      ...
      const result = await this.paystack('/transaction/initialize', { method: 'POST', body: JSON.stringify(payload) });
  ...
  validateSuccessfulPayment(intent, data) {
    if (!data || data.status !== 'success') throw Object.assign(new Error('The transaction has not completed successfully.'), { statusCode: 409 });
  ...
  async verify({ account, reference }) {
    const wallet = await this.store.ensureOwnedWallet(account);
    const intent = await this.store.readIntent(reference);
    if (intent.accountId !== account.id || intent.walletId !== wallet.id) {
      throw Object.assign(new Error('This payment belongs to another account.'), { statusCode: 403 });
    }
    if (intent.credited) return { status: 'success', wallet: await this.store.getWalletForAccount(account), reference };
  ...
  async handleWebhook(rawBody, signature) {
    ...
    if (!this.verifyWebhookSignature(rawBody, signature)) throw Object.assign(new Error('Invalid Paystack webhook signature.'), { statusCode: 401 });
    let event;
    try {
