...
function recoveryReference(planId) {
  const value = String(planId || '');
  return value.startsWith('recover:') ? value.slice('recover:'.length).trim() : '';
}
...
export class PaystackPayments {
  ...
  async initialize({ token, email, planId }) {
    const cleanReceiptEmail = cleanEmail(email);
    const referenceToRecover = recoveryReference(planId);
    if (referenceToRecover) return this.recover({ email: cleanReceiptEmail, reference: referenceToRecover });

    const wallet = await this.store.authenticate(token);
    const existingWalletId = await this.store.accountWalletId(cleanReceiptEmail);
    if (existingWalletId && existingWalletId !== wallet.id) {
      throw Object.assign(new Error('Credits already exist for this email. Recover the account using a Paystack receipt reference before buying more.'), { statusCode: 409 });
    }
  ...
  async verify({ token, reference }) {
    ...
    if (intent.walletId !== wallet.id) throw Object.assign(new Error('This payment belongs to another credit account.'), { statusCode: 403 });
  ...
  async recover({ email, reference }) {
    const cleanReceiptEmail = cleanEmail(email);
    const intent = await this.store.readIntent(reference);
    if (String(intent.email || '').toLowerCase() !== cleanReceiptEmail) {
      throw Object.assign(new Error('The email and Paystack receipt reference do not match.'), { statusCode: 403 });
    }

    const result = await this.paystack(`/transaction/verify/${encodeURIComponent(reference)}`, { method: 'GET' });
    this.validateSuccessfulPayment(intent, result.data);
    const paystackEmail = String(result.data?.customer?.email || '').trim().toLowerCase();
    if (paystackEmail && paystackEmail !== cleanReceiptEmail) {
      throw Object.assign(new Error('The email and Paystack receipt reference do not match.'), { statusCode: 403 });
    }

    const credited = intent.credited
      ? { intent, wallet: null }
      : await this.store.creditPayment(reference, result.data);
    const walletId = credited.intent?.walletId || intent.walletId;
    const recovered = await this.store.recoverAccount(cleanReceiptEmail, walletId);
    return {
      recovered: true,
