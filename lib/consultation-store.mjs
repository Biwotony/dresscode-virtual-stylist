...
export class ConsultationStore {
  ...
  async authenticate(token) {
    const studioId = this.parseStudioToken(token);
    const studio = await this.readJson(join(this.studioDir(studioId), 'studio.json'));
    if (!studio || !safeHashEqual(studio.tokenHash, hash(token))) {
      throw Object.assign(new Error('Studio access is missing or invalid.'), { statusCode: 401 });
    }
    return studio;
  }
  ...
  async updateStudio(token, profile) {
  ...
  async createClient(token, input, modelBuffer = null) {
    const studio = await this.authenticate(token);
    const id = randomUUID();
    const createdAt = now();
    const client = {
  ...
  async getClient(token, clientId) {
    const studio = await this.authenticate(token);
  ...
  async updateClient(token, clientId, input, modelBuffer = null) {
    const studio = await this.authenticate(token);
    const client = await this.getClient(token, clientId);
    Object.assign(client, structuredClone(input), { updatedAt: now() });
    if (modelBuffer) {
  ...
  async readClientModel(token, clientId) {
    const studio = await this.authenticate(token);
    const client = await this.getClient(token, clientId);
    if (!client.modelFile) return null;
    return readFile(join(this.clientDir(studio.id, clientId), client.modelFile));
  }

  async listConsultations(token) {
    const studio = await this.authenticate(token);
  ...
  async createConsultation(token, input, inspirationBuffer = null) {
    const studio = await this.authenticate(token);
    const client = await this.getClient(token, input.clientId);
    const id = randomUUID();
    const createdAt = now();
    const consultation = {
      id,
  ...
  async getConsultation(token, consultationId) {
    const studio = await this.authenticate(token);
  ...
  async updateConsultation(token, consultationId, input, inspirationBuffer = null) {
    const studio = await this.authenticate(token);
    const consultation = await this.getConsultation(token, consultationId);
    if (input.clientId && input.clientId !== consultation.clientId) await this.getClient(token, input.clientId);
    ...
    const client = await this.getClient(token, consultation.clientId);
    await this.saveConsultationIndex(studio.id, this.consultationSummary(consultation, client));
    return consultation;
  }
  ...
  async readConsultationInspiration(token, consultationId) {
    const studio = await this.authenticate(token);
    const consultation = await this.getConsultation(token, consultationId);
    if (!consultation.inspirationFile) return null;
    return readFile(join(this.consultationDir(studio.id, consultationId), consultation.inspirationFile));
  }

  async addVersion(token, consultationId, input, imageBuffer) {
    const studio = await this.authenticate(token);
    const consultation = await this.getConsultation(token, consultationId);
    const id = randomUUID();
  ...
  async readPrivateVersion(token, consultationId, versionId) {
    const studio = await this.authenticate(token);
    const consultation = await this.getConsultation(token, consultationId);
  ...
  async createShare(token, consultationId) {
    const studio = await this.authenticate(token);
    const consultation = await this.getConsultation(token, consultationId);
    if (!consultation.versions?.length) {
      throw Object.assign(new Error('Save at least one design version before sharing.'), { statusCode: 409 });
    }
    const shareToken = `${studio.id}.${consultation.id}.${randomBytes(24).toString('base64url')}`;
    consultation.shareHash = hash(shareToken);
    consultation.approval = {
      status: 'pending',
      versionId: consultation.versions.at(-1).id,
      comment: '',
      decidedAt: null,
      clientName: ''
    };
    ...
    return {
      ...
      consultation: {
        ...
        brief: consultation.brief ? {
          event: consultation.brief.event || '',
          garment: consultation.brief.garment || '',
          fit: consultation.brief.fit || '',
          fabric: consultation.brief.fabric || '',
          colour: consultation.brief.colour || '',
          idea: consultation.brief.idea || ''
        } : null,
        approval: structuredClone(consultation.approval),
        order: {
          currency: consultation.order?.currency || 'KES',
          quoteAmount: consultation.order?.quoteAmount || 0,
          depositAmount: consultation.order?.depositAmount || 0,
          depositStatus: consultation.order?.depositStatus || 'not_recorded',
          orderStatus: consultation.order?.orderStatus || 'consultation',
          dueDate: consultation.order?.dueDate || ''
        },
  ...
  async recordDecision(token, input) {
    ...
    if (status === 'approved' && consultation.order.orderStatus === 'consultation') {
      consultation.order.orderStatus = 'design_approved';
    }
    consultation.updatedAt = now();
    await this.writeJson(join(this.consultationDir(studio.id, consultation.id), 'consultation.json'), consultation);
    await this.saveConsultationIndex(studio.id, this.consultationSummary(consultation, client));
  ...
  async updateOrder(token, consultationId, order) {
    const studio = await this.authenticate(token);
    const consultation = await this.getConsultation(token, consultationId);
