...
export class PaymentStore {
  ...
  async readAccount(email) {
    try {
      return JSON.parse(await readFile(this.accountPath(email), 'utf8'));
    } catch (error) {
  ...
  async accountWalletId(email) {
    return (await this.readAccount(email))?.walletId || null;
  }
  ...
  async ensureAccount(email, requestedWalletId) {
    const cleanEmail = normaliseEmail(email);
    if (!cleanEmail) return requestedWalletId;
    const key = hashSecret(cleanEmail);
    return this.withLock(`account:${key}`, async () => {
      const existing = await this.readAccount(cleanEmail);
      const walletId = existing?.walletId || requestedWalletId;
      const wallet = await this.readWallet(walletId);
      if (wallet.email !== cleanEmail) {
        wallet.email = cleanEmail;
  ...
  async rotateWalletSecret(walletId) {
    return this.withLock(`wallet:${walletId}`, async () => {
      const wallet = await this.readWallet(walletId);
      const secret = randomBytes(32).toString('base64url');
      wallet.secretHash = hashSecret(secret);
      wallet.ledger.push({
        id: randomUUID(),
        type: 'security',
        amount: 0,
        reference: `recovery:${Date.now()}`,
        description: 'Credit account recovered on a new browser',
        createdAt: new Date().toISOString()
      });
      await this.saveWallet(wallet);
      return { token: `${wallet.id}.${secret}`, wallet: publicWallet(wallet) };
    });
  }

  async recoverAccount(email, walletId) {
    const accountWalletId = await this.ensureAccount(email, walletId);
    return this.rotateWalletSecret(accountWalletId);
  }
  ...
  async creditPayment(reference, paymentData = {}) {
    const intent = await this.readIntent(reference);
    const targetWalletId = await this.ensureAccount(intent.email, intent.walletId);
    return this.withLock(`wallet:${targetWalletId}`, async () => {
      const freshIntent = await this.readIntent(reference);
      if (freshIntent.walletId !== targetWalletId) {
        freshIntent.originalWalletId ||= freshIntent.walletId;
        freshIntent.walletId = targetWalletId;
      }
      const wallet = await this.readWallet(targetWalletId);
