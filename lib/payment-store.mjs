...
export class PaymentStore {
  ...
  async claimWallet(token, accountId, email = '') {
    try {
      ...
      if (wallet.ownerAccountId && wallet.ownerAccountId !== accountId) {
        return (await this.createWallet({ ownerAccountId: accountId, email })).wallet;
      }
  ...
  async mergeLegacyWallet(token, targetWalletId, accountId, email = '') {
    let source;
    try {
      source = await this.authenticate(token);
    } catch (error) {
      if (error.statusCode !== 401) throw error;
      return this.claimWalletById(targetWalletId, accountId, email);
    }
    if (source.ownerAccountId && source.ownerAccountId !== accountId) return this.claimWalletById(targetWalletId, accountId, email);
